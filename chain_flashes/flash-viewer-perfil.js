...
.cfv-media-container video,
.cfv-media-container img {
    width: 100%;
    height: 100%;
    object-fit: cover;
}
                ...
                .cfv-reaction-btn:hover {
                    transform: scale(1.1);
                    border-color: rgba(255, 255, 255, 0.6);
                }

                .cfv-reaction-btn.reacted {
                    background: linear-gradient(135deg, #6366f1, #ec4899);
                    border-color: white;
                ...
                .cfv-action-btn:hover {
                    background: rgba(255, 255, 255, 0.2);
                    transform: translateY(-2px);
                }

                .cfv-action-btn.danger {
                    border-color: rgba(239, 68, 68, 0.5);
                    color: #ef4444;
                }

                .cfv-action-btn.primary {
...
// ✅ PRIORIDAD 2: SWIPE HORIZONTAL (solo en modo global)
    if (isSwiping) {
        // ✅ NO permitir swipe en modo usuario específico
        if (this.singleUserMode) {
            return; // Ignorar swipe
        }
...
this.viewerElement.addEventListener('touchmove', handleMove, { passive: true });
...
async open(flashId) {
    ...
    try {
        ...
        console.log('📡 Cargando flashes del usuario...');
...
async loadAllFlashes() {
    try {
        // ✅ DETECTAR SI ESTAMOS EN PÁGINA DE USUARIO ESPECÍFICO
        const urlParams = new URLSearchParams(window.location.search);
        const specificUser = urlParams.get('user') || urlParams.get('user_id') || urlParams.get('username');
        
        let response;
        if (specificUser) {
            // ✅ MODO USUARIO ESPECÍFICO - Solo cargar sus flashes
            console.log('👤 Modo usuario específico:', specificUser);
            this.singleUserMode = true;
            response = await fetch(`/php/chain_flashes/obtener_flashes_usuario.php?user_id=${encodeURIComponent(specificUser)}`);
        } else {
            // ✅ MODO NORMAL - Cargar todos los flashes
            console.log('🌍 Modo global (inicio)');
            this.singleUserMode = false;
            response = await fetch('/php/chain_flashes/obtener_flashes.php');
        }
        ...
        if (data.success) {
            ...
            console.log('📊 Modo:', this.singleUserMode ? 'Usuario específico' : 'Global');
...
loadFlashContent() {
 ...
 updateHeader(flash) {
    ...
    if (timeAgo) {
        // ✅ NUEVO: Tiempo + Badge de anclado inline
...
if (flash.tipo === 'video') {
    ...
    // ✅ NUEVO: Prevenir reproducción cuando el panel está abierto
...
resumeProgress() {
    ...
    // ✅ SI NO HAY MÁS EN ESTE USUARIO, IR AL USUARIO ANTERIOR
        ...
        // Ir al último flash del usuario anterior
...
async navigateNext() {
    ...
    // ✅ SI ES EL ÚLTIMO USUARIO Y ÚLTIMO FLASH
    else {
        if (this.singleUserMode || this.currentUserIndex >= this.userList.length - 1) {
            console.log('✅ Fin de flashes - cerrando viewer');
            this.close();
            return;
        }
    }
...
async reactFromPanel(emoji, event) {
    ...
    btn.disabled = true;
    btn.innerHTML = `<div class="reaction-spinner"></div>`;
...
showBottomPanel() {
    ...
    // ✅ NUEVO: Si es visitante, expandir automáticamente a 45vh
